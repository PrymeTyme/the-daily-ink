...
def main():
    ...
    rendered_html = rendered_html.replace("{movie_rating}", str(movie_data['rating']))
    ...
    # 2. Generate the absolute local file URL (import os removed from here!)
    absolute_path = os.path.abspath(poster_index_path)
    file_url = f"file:///{absolute_path.replace(chr(92), '/')}"
    
    # 3. Take the screenshot using the absolute URL
    hti.screenshot(
        url=file_url, 
        save_as='poster.png',
        size=(480, 800)
